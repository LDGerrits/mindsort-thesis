...
/**
 * @class AdaptiveContrastingAlgorithm
 * @extends Contrasting
 * @description This class represents an adaptive contrasting algorithm that extends the base Contrasting class.
 */
export class AdaptiveContrastingAlgorithm extends Contrasting {
    ...
    updateToDo(): ItemPair[] {
        ...
        if (this.initialized) {
            ...
            // Increase contrasting level if answer was correct (only works for Absolute Score)
            if (this.totalScore !== this.gameState.totalScore) {
                this.totalScore = this.gameState.totalScore;
                
                // Increase
                if (this.currentRound !== 0) {
                    this.currentItem.contrastingLevel = Math.min(this.currentItem.contrastingLevel + 1, 2);
                }
            }
        }
        else {
        ...
        this.currentItem.countSeen += 1;
        todo.push(this.currentItem.itemPair);

        let secondItem = this.getItemFromItemPair(this.getItemPairOfContrastingLevel(this.currentItem, this.currentItem.contrastingLevel))!;
        secondItem.countSeen += 1;
        todo.push(secondItem.itemPair);

        let thirdItem = this.getItemFromItemPair(this.getOverlappingItempair(this.currentItem, secondItem, this.currentItem.contrastingLevel))!; 
        thirdItem.countSeen += 1;
        todo.push(thirdItem.itemPair);

        this.gameState.customData.set("contrastingLevel", this.currentItem.contrastingLevel);
        this.gameState.customData.set("contrastedItemPairs", todo);
